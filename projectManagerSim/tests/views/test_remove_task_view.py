...
class RemoveTaskViewTestCase(TestCase):
    ...
    def test_remove_task_success(self):
        ...
        self.assertIn("task_assign_menu_html", data)

        self.save_character.refresh_from_db()
        self.assertEqual(self.save_character.task_assigned, None)
    ...
    def test_missing_character_id(self):
        ...
        self.assertEqual(response.json()["message"], "Missing save_character_id")

    def test_save_character_not_found(self):
        self.client.login(username="testuser", password="testpass123")
        ...
        self.assertFalse(response.json()["success"])
        self.assertEqual(response.json()["message"], "Character not found")

    def test_save_character_not_allowed(self):
        self.client.login(username="testuser", password="testpass123")
        ...
        self.assertFalse(response.json()["success"])
        self.assertEqual(response.json()["message"], "Not allowed to remove this task")
    ...
    @patch("projectManagerSim.views.assign_task_view.SaveCharacter.save")
